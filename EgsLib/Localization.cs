...
namespace EgsLib
{
    ...
    {
        private static readonly StringPool StringPool = new StringPool(64);
        ...
        public string Localize(string key, string language)
        {
            ...
            {
                if (map.TryGetValue(language, out string text) && text != null)
                    return text;
            }
        ...
        private static Dictionary<string, IReadOnlyDictionary<string, string>> ParseData(string contents)
        {
            ...
            {
                ColumnStringFactory = StringPool.GetString
            };
